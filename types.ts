...
export interface AdminAreaLeader {
  id: string;
  name: string;
  title: string; // e.g. Governor, CAO, District Chairperson, RDC, Sub-County Chief
  phone: string;
  email: string;
  term?: string; // e.g. "2021 - 2026"
  department?: string; // e.g. "Administration", "Health Services", "Finance"
  photoUrl?: string;
  status?: 'Active' | 'On Leave' | 'Acting';
}

export interface AdminAreaOffice {
  officeName?: string; // e.g. "Mukono District Local Government Headquarters"
  building?: string; // e.g. "District Administration Block, 2nd Floor"
  street?: string; // e.g. "Plot 14 Jinja-Kampala Highway"
  townOrCity?: string; // e.g. "Mukono Town"
  postalCode?: string; // e.g. "P.O. Box 110"
  phone?: string;
  email?: string;
  coordinates?: {
    latitude: number;
  ...
  workingHours?: string; // e.g. "Mon - Fri: 08:00 AM - 05:00 PM"
}
...
export type ActiveView = 'dashboard' | 'dashboard-users' | 'dashboard-system' | 'dashboard-traffic' | 'dashboard-products' | 'dashboard-customer' | 'customer-make-order' | 'customer-purchases' | 'customer-loyalty' | 'profile' | 'settings' | 'settings-cameras' | 'settings-api' | 'settings-calls' | 'id-verification' | 'stock-listing' | 'inventory' | 'countries' | 'country-profile' | 'countries-map' | 'country-admin-levels' | 'country-locations-upload' | 'country-electoral-levels' | 'regional-economic-levels' | 'shop-users' | 'super-users' | 'permissions' | 'stock-purchase' | 'roles' | 'shops' | 'shop-profile' | 'shop-surveillance' | 'wallet-settings' | 'finances-income-statement' | 'finances-balance-sheet' | 'finances-cash-flow' | 'income-statement-detail' | 'balance-sheet-detail' | 'cash-flow-detail' | 'reports-messages' | 'reports-calls' | 'reports-daily-sales' | 'reports-stock-level' | 'reports-product-profile' | 'reports-packet-tracer' | 'reports-packet-tracer-live' | 'reports-packet-tracer-config' | 'client-list' | 'client-wallets' | 'client-loyalty' | 'admin-loyalty-mgt' | 'system-message-settings' | 'product-chain-products' | 'product-chain-manufacturers' | 'product-chain-distributors' | 'product-chain-suppliers' | 'sales-desk' | 'mobi-agent-settings' | 'mno-wallet-settings' | 'mno-wallet-transactions' | 'exchange-rate' | 'lookup-values';
