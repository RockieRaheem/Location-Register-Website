...
const Sidebar: React.FC<SidebarProps> = ({ theme, toggleTheme, activeView, onNavigate, isCollapsed, allowCalls, onLogout, userRole }) => {
  ...
  const isCountriesMgtMenuActive = ['countries', 'country-admin-levels', 'country-locations-upload', 'country-electoral-levels', 'countries-map', 'regional-economic-levels', 'exchange-rate'].includes(activeView);
                    ...
                    <div>
                        ...
                        <div className={`overflow-hidden transition-all duration-300 ease-in-out ${isCountriesMgtMenuOpen && !isCollapsed ? 'max-h-[500px]' : 'max-h-0'}`}>
                            <div className={`pt-1 space-y-1 ${isCollapsed ? 'hidden' : 'pl-4'}`}>
                                ...
                                <NavLink isCollapsed={isCollapsed} theme={theme} icon="upload" label={t('sidebar.countryLocationsUpload', 'Upload Locations')} active={activeView === 'country-locations-upload'} onClick={() => onNavigate('country-locations-upload')} />
                            </div>
                        </div>
                    </div>
                )}
