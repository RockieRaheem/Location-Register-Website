...
import { mockVerificationRequests, mockShopRoles, mockSuperUserRoles, mockShopUsers as initialMockShopUsers, allAfricanCountries, mockShops as initialMockShops, mockNotifications as initialMockNotifications, mockCameraDevices, mockCallRecords, mockProductDefinitions, mockStockListings, mockRegionalEconomicLevels } from '../../data/mockData';
...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ onLogout, userRole = 'Customer' }) => {
  ...
  useEffect(() => {
    ...
    const unsubProducts = subscribeToProducts((data) => {
      if (data && data.length > 0) {
        ...
        // If DB is empty, use mock data in state
        setProductsState(mockProductDefinitions);
      }
    });
    ...
    const unsubStock = subscribeToStockItems((data) => {
      if (data && data.length > 0) {
        ...
        // Use mock data in state if empty
        setStockItemsState(mockStockListings);
      }
    });
    ...
    const unsubRegional = subscribeToRegionalLevels((data) => {
      if (data && data.length > 0) {
        ...
        setRegionalLevelsState(mockRegionalEconomicLevels);
      }
    });
  ...
  const setProducts: React.Dispatch<React.SetStateAction<ProductDefinition[]>> = (value) => {
    if (typeof value === 'function') {
      ...
      if (firebaseUser) {
        // Persist modified items to Firebase if authenticated
        next.forEach(p => {
  ...
  const setStockItems: React.Dispatch<React.SetStateAction<StockItem[]>> = (value) => {
    if (typeof value === 'function') {
      ...
      if (firebaseUser) {
        next.forEach(s => {
          const old = stockItems.find(o => o.id === s.id);
