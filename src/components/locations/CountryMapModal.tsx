...
import { countryDetailedMaps } from '../../data/maps/generated/countryPaths';
...
const CountryMapModal: React.FC<CountryMapModalProps> = ({ 
  ...
  const viewBox = useMemo(() => {
    if (countryId === 'UG' && selectedDistrict && currentLevel === 'villages') {
      if (selectedDistrict === 'Kampala') {
        return selectedDivision === 'Kawempe' ? "110 15 220 250" : "0 0 500 580";
      }
      if (UGANDA_DISTRICTS_DATA[selectedDistrict]) {
        return "0 0 600 600";
      }
    }
          ...
          <div className={`flex items-center justify-between p-4 sm:p-5 border-b shrink-0 ${theme === 'dark' ? 'border-slate-800 bg-slate-900/40' : 'border-slate-200 bg-white'}`}>
            ...
            <div className="flex items-center space-x-2">
              ...
              <button
                ...
                id="country-modal-profile-btn"
                className={`flex items-center space-x-1.5 px-3 py-1.5 rounded-lg text-xs font-bold transition-all border ${
                  theme === 'dark' 
                    ? 'border-yellow-500/40 bg-yellow-500/10 hover:bg-yellow-500/20 text-yellow-400' 
                    : 'border-yellow-500/40 bg-yellow-50 hover:bg-yellow-100 text-yellow-700'
                }`}
                title={`Access ${countryName} Profile`}
              >
                <Eye size={15} />
                <span className="hidden sm:inline">Country Profile</span>
              </button>
                        ...
                        {countryId === 'TZ' && selectedDistrict === 'Dar es Salaam' ? (
                          ...
                          selectedDistrict === 'Kampala' ? (
                            ...
                            UGANDA_DISTRICTS_DATA[selectedDistrict]?.subdivisions.map((sub, idx) => {
                              ...
                              const strokeColor = isHovered ? '#3b82f6' : (theme === 'dark' ? '#64748b' : '#94a3b8');

                              return (
                                <g key={`modal-ug-sub-${idx}`} id={`modal-sub-${sub.pcode || idx}`}>
                                  <motion.path
                                    d={sub.d}
                                    fill={fillColor}
                                    stroke={strokeColor}
                            ...
                            return (
                              <g key={path.id} className="outline-none">
                                <motion.path
                                  ...
                                  fill={fillColor}
                                  stroke={strokeColor}
                                  strokeWidth={countryId === 'TZ' || countryId === 'UG' ? (isHovered ? "2.2" : "1.2") : (isHovered ? "1.8" : "0.9")}
                                  strokeLinejoin="round"
                                  strokeLinecap="round"
                                  opacity={opacity}
