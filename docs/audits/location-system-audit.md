...
The trusted migration script reads the already validated SQLite registry and preserves all UUIDs, source provenance, hierarchy definitions, parent relationships, and ancestor paths. It is dry-run by default; no cloud data is written unless `--commit` and a numbered batch are supplied. The exact registered `Any-Location` Web SDK configuration is stored in ignored `.env.local`. The `(default)` Firestore database is now active in `africa-south1`, Standard edition, with its free tier enabled. To respect Spark's 20,000-write daily quota, imports are restricted to 15,000-location batches. Cloud Storage calls and deployment are disabled because Firebase does not provide Cloud Storage access on Spark. Full procedures are in [`FIREBASE_INTEGRATION.md`](./FIREBASE_INTEGRATION.md).
