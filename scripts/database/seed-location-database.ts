...
import { allAfricanCountries } from '../../src/data/mockData.ts';
import electoralCommissionData from '../../data/sources/uganda/electoral-commission-2022.json';
import { UGANDA_DISTRICTS_DATA } from '../../src/data/maps/generated/ugandaDistrictsData.ts';
import {
...
} from '../../src/data/locations/ugandaElectoralCommission2022.ts';
import {
  deterministicUuid,
  LocationDatabase,
...
import type { AdminLevelName, Country } from '../../src/types.ts';

const projectRoot = path.resolve(import.meta.dirname, '../..');
const databasePath = path.resolve(process.argv.find((argument) => argument.endsWith('.sqlite')) || path.join(projectRoot, 'data', 'database', 'location-register.sqlite'));
...
try {
  ...
  fs.writeFileSync(path.join(projectRoot, 'data', 'reports', 'location-database.seed-report.json'), `${JSON.stringify(report, null, 2)}\n`);
